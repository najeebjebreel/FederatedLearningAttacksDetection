...
class Peer():
    ...
    peer_type = 'Honest', mapping_list = None, malicious_behaviour_rate = 0):
    ...
    def train_and_update(self, model, dataset_fraction, global_epoch, maximum_attacks) :
        ...
        if(self.peer_type == 'Random_Attacker' and type(self).performed_attacks<maximum_attacks):
            ...
            if(r <= self.malicious_behaviour_rate):
                ...
                for key in w.keys():
                    noise = (torch.tensor(np.random.normal(0, 0.03, size=w[key].size()))).to(self.device)
                    w[key]+= noise
...
class SimpleEnv:
    ...
    attack_type = None, attack_rate = None, malicious_behaviour_rate = None, mapping_list = None, maximum_attacks=0):
        ...
        os.chdir("D:\Federated_Learning\Applications\Experiments of March2020\Federated_Learning_EigentTrust_100_workers_GPU\mnist/")
        ...
        if not (os.path.exists(os.path.join(self.PATH, 'MNIST_global_model.pth'))):
            torch.save(self.global_model.state_dict(), os.path.join(self.PATH, 'MNIST_global_model.pth'))
        else:
            self.global_model.load_state_dict(torch.load(os.path.join(self.PATH, 'MNIST_global_model.pth')))
    ...
    def split_dataset(self):
        ...
        train_path = str(os.getcwd()+"/datasets/mnist/training/")
        test_path = str(os.getcwd() + "/datasets/mnist/testing")
        ...
        self.global_train_dataset = Dataset(train_path, transform_train, 0, 59600, 28, False)
        self.global_test_dataset = Dataset(test_path, transform_test,  0, 10000, 28, False) 


        for i in range(self.num_peers):
            self.training_dataset_list.append(Dataset(train_path, transform_train, i*596, (i+1)*596, 28, True))
...
class SecureEnv(SimpleEnv):
    ...
    def __init__(self, num_peers, peer_pseudonyms, peer_frac, seed, tau, global_rounds, local_epochs, local_bs, local_lr,
    local_momentum, num_classes, labels_dict, classes_list, device, 
    attack_type, attack_rate, malicious_behaviour_rate,  mapping_list, maximum_attacks=0):
        ...
        super().__init__(num_peers = num_peers, peer_pseudonyms = peer_pseudonyms, peer_frac = peer_frac, seed = seed, tau = tau, 
            ...
            attack_type = attack_type, attack_rate = attack_rate, mapping_list = mapping_list, malicious_behaviour_rate = malicious_behaviour_rate, maximum_attacks=maximum_attacks)
    ...
    def display_current_credits(self):
    ...
    def simulate(self, sim_num):
        ...
        for epoch in tqdm_notebook(range(self.global_rounds)):
            ...
            last_layer_biases = get_layer_weights(local_weights, layer_name = 'fc2.bias')
